...
func GenPhiB19(session string) ([]byte, error) {
	...
	f, _ := os.Create("./xassets/gamesave.json")
	f.Write(data)
	...
	b19, err := DrawB19(1, j, strconv.FormatFloat(float64(j.Summary.Rks), 'f', 6, 64), challengemoderank[(j.Summary.ChallengeModeRank-(j.Summary.ChallengeModeRank%100))/100], strconv.Itoa(int(j.Summary.ChallengeModeRank%100)), "10001")
	if err != nil {
		return nil, fmt.Errorf("failed to generate b19: %w", err)
	}
	return b19, nil
}

func DrawB19(accuracy float64, j phigros.UserRecord, allrks, chal, chalnum, uid string) ([]byte, error) {
	var (
	...
	drawfile, err := os.ReadDir(respath + Illustration)
	if err != nil {
		return nil, fmt.Errorf("failed to load illustration: %w", err)
	}
	imgs, err := gg.LoadImage(respath + Illustration + drawfile[rand.Intn(len(drawfile))].Name())
	if err != nil {
		return nil, fmt.Errorf("failed to load illustration: %w", err)
	}
	...
	if chal != "" {
		chall, err := gg.LoadPNG(respath + Challengemode + chal + ".png")
		if err != nil {
			return nil, fmt.Errorf("failed to load challenge mode: %w", err)
		}
	...
	go func() {
		defer wg.Done()
		logo, err := gg.LoadImage(respath + uid + "/transParen.png")
		if err != nil {
			return
		}
...
func drawcardback(accuracy float64, i, w int, a float64, list phigros.ScoreAcc) (img image.Image, err error) {
	...
	rankim, err := gg.LoadImage(respath + Rank + rank + ".png")
	if err != nil {
		return
	}
	...
	_, err = os.Stat(respath + Illustration + list.SongId + ".0.png")
	if list.SongId != "" && (err == nil || os.IsExist(err)) {
		var imgs image.Image
		imgs, err = gg.LoadImage(respath + Illustration + list.SongId + ".0.png")
		if err != nil {
			return
		}
